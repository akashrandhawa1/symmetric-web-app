...
export async function fetchHomeHeroCopy(
    ...
    // Build offline plan to get reference movements
    const offlinePlanForHero = buildOfflineWorkoutPlan(context ?? 'generalPrep');
    const referenceMovements = offlinePlanForHero.blocks
...
export async function fetchDailyWorkoutPlan(
    ...
    if (intakeSummaryLines.length) {
        ...
        if (insertionIndex >= 0) {
            promptLines.splice(insertionIndex, 0, ...inferenceRules);
            console.log('[DailyWorkoutPlan] ✅ Intake profile + inference rules injected at line', insertionIndex);
        } else {
            promptLines.push(...inferenceRules);
            console.log('[DailyWorkoutPlan] ✅ Intake profile + inference rules appended to end of prompt');
        }
    } else {
